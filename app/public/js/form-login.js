    ...
    document.querySelector("form").addEventListener("submit", function (event) {
        ...
        Array.from(form).forEach(function (element) {
            ...
            if (elementType == "password") {
                resultElementTypeValidator.push({
                    type: "password",
                    value: validatorPassword(element),
                    id: id
                });
            }
...
function validatorPassword(element) {
...
function regexText(text) {
    let letraMaiuscula = /[A-Z]/.test(text);
    let temNumero = /\d/.test(text);
    let caracterEspecial = /[! @ % # $ ¨ ^ & * ( ) _ - = \ ]/.test(text);
    let comprimentoSuficiente = text.length >= 4;

    if(letraMaiuscula && temNumero && caracterEspecial && comprimentoSuficiente)
    {
        return true
    }
